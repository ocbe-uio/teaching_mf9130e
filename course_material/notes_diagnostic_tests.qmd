...
|---------------------------|-----------------------|-----------------------|------------|
| **With condition**        | True Positive TP      | False Negative FN     |    $P$     |
| **Without condition**     | False Positive FP     | True Negative TN      |    $N$     |
| **Total**                  |$P_{\text{predicted}}$  | $N_{\text{predicted}}$|
...
-   Specificity: $P(\text{neg}|\text{well})$
-   Positive predictive value: $P(\text{ill}|\text{pos})$
-   Negative predictive value: $P(\text{well}|\text{neg})$
...
$$
\begin{aligned}
PPV = P(\text{ill}|\text{pos}) & = \frac{P(\text{pos}|\text{ill}) \cdot P(\text{ill})}{P(\text{pos})}\\
                               & = \frac{P(\text{pos}|\text{ill}) \cdot P(\text{ill})}{P(\text{pos}|\text{ill}) \cdot P(\text{ill}) +  P(\text{pos}|\text{well}) \cdot P(\text{well})}\\

 & =\frac{\text{sens} \cdot \text{prev}}{\text{sens} \cdot \text{prev} + (1-\text{spes}) \cdot (1-\text{prev})}
\end{aligned}
$$
