...
The output indicates a very significant correlation between `pefsit2` and `pefsit1 (p < 0.001)`. This could be expected from observing the scatterplot.
...
This again shows a very small p-value (p < 0.001), which indicates that the correlation between `pefsit1` and `weight` is significant. However, the value of the correlation is slightly lower than for `pefsit2` and `pefsit1`, and this corresponds to the slightly worse fit of the line in the scatter plot.
...
R^2 = 93.96\%. The model is highly statistically significant.
