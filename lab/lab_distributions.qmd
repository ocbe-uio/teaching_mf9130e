...
# we compute the frequency distribution of the samples and store them in variable data
...
The probability density function of the normal distribution with mean $\mu$ and standard deviation $\sigma$ is given by:
...
#note that this function is already define in R as dnorm(x), we could use this instead
...
The histogram would look closer to a bell shape the larger the number of samples. We can also check if our data are normally distributed using the command `qqnorm(data)` that generates a QQPlot. These plots display the data quantiles against the theoretical quantiles of the normal distribution. Therefore, normally distributed data should lie close to the identity line $y=x$.
